...
function crossReferenceListsAndFaqs(lists, faqs) {
  lists.forEach(list => {
    const childRefs = normalizeReferenceIds(list._yaml.faqs, list.isRoot ? null : list.id);
...
function resolveLinksAndRender(items, markdownField, htmlField, internalLinkIndex) {
  items.forEach(item => {
    // First resolve links in the markdown
    item[markdownField] = resolveLinks(item[markdownField], item._directory, internalLinkIndex, craReferences);
    // Then render to HTML
